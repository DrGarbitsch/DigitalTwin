...
Nine of these rules are now enforced by an executable suite rather than only
described here: `validation/specs/opc-10000-3-address-space/` holds a SHACL shape per rule plus
two passing and two failing NodeSet2 fixtures for each, run by
`validation/run_suite.py` and by `make test`. Those rows are marked **Enforced
(suite)** below and name their shape and fixture directory.

The shapes are written against what `nodeset2owl.py` actually emits, which is not
the vocabulary the older `validation/ontology/*.shacl.ttl` shapes assume; see
`validation/README.md` for the differences that matter when implementing the next
rule, and for how to add one.

Everything else in this catalog is still a description. The remaining rows are the
backlog, with the reason each one is open recorded in its Notes.
...
| **Enforced (suite)** | A SHACL shape in `validation/specs/opc-10000-3-address-space/shapes/` checks this, backed by two passing and two failing NodeSet2 fixtures under `validation/specs/opc-10000-3-address-space/testcases/<ID>/`. Written against what `nodeset2owl.py` actually emits and run by `validation/run_suite.py` (also part of `make test`). See `validation/README.md`. |
...
| ID | Section | Rule | Status | Notes |
|---|---|---|---|---|
| AS-005 | 4.5.2, **5.6.3** | A Property (a Node reached via `hasProperty`) must never itself be the SourceNode of **any hierarchical Reference** — not just `hasProperty`. It may still be the SourceNode of NonHierarchical References. | **Enforced (suite)** | Shape `validation/specs/opc-10000-3-address-space/shapes/AS-005-property-not-hierarchical-source.shacl.ttl`, with 2 passing and 2 failing nodesets under `validation/specs/opc-10000-3-address-space/testcases/AS-005/`. Supersedes `NoHasPropertyOnVariableNodeShape`, which only covered the `hasProperty` case; the shape now tests the ReferenceType against `opcua:HierarchicalReferences`, so `hasComponent`/`organizes` out of a Property are caught too, and NonHierarchical References out of a Property stay legal. *Pass 3 upgrade:* 5.6.3 gives the broader, precise rule — "Properties shall not be the SourceNode of any hierarchical References" / "may be the SourceNode of any NonHierarchical References" — wider than pass 1's `hasProperty`-only wording. `validation/ontology/hasProperty.shacl.ttl` → `NoHasPropertyOnVariableNodeShape` only checks the `hasProperty` case, not `hasComponent`/`organizes`/`hasSubtype`/etc. — narrower than the now-confirmed full rule. Independently corroborated by §7 (AS-032's source text: "a Property shall never be the SourceNode of a HasProperty Reference"). |
| AS-006 | 4.5.2 | For a given parent Node, all of its direct Properties (`hasProperty` targets) must have distinct BrowseNames (uniqueness scoped to Property-siblings of the same source). | **Enforced (suite)** | Shape `validation/specs/opc-10000-3-address-space/shapes/AS-006-sibling-property-browsenames.shacl.ttl`, with 2 passing and 2 failing nodesets under `validation/specs/opc-10000-3-address-space/testcases/AS-006/`. BrowseNames are compared as (name, namespace) pairs and scoped to one owner. Was: not present in `hasProperty.shacl.ttl`. Generalized by AS-009 (below) for Type/InstanceDeclaration nodes specifically, but this rule applies to *any* node with Properties, not just types. |
...
| AS-039 | 6.2.1 | An **instance's** `HasTypeDefinition` target must never be abstract (`IsAbstract=true`) — even though the InstanceDeclaration it derives from may legitimately point to an abstract type. | **Enforced (suite)** | Shape `validation/specs/opc-10000-3-address-space/shapes/AS-039-instance-type-not-abstract.shacl.ttl`, with 2 passing and 2 failing nodesets under `validation/specs/opc-10000-3-address-space/testcases/AS-039/`. The InstanceDeclaration exemption is carried by the shape: nodes carrying a `HasModellingRule` are exempt. *Pass 2 finding.* "The type of an InstanceDeclaration may be abstract, however the instance must be of a concrete type" (6.2.1). Connects AS-008/AS-010's HasTypeDefinition rules to the `IsAbstract` Attribute from AS-015/AS-017/AS-018/AS-021. |
...
| ID | Section | Rule | Status | Notes |
|---|---|---|---|---|
| AS-031 | 7 | `HasComponent`: SourceNode must be Object, ObjectType, DataVariable, or VariableType; TargetNode must be Variable, Object, or Method. | **Enforced (suite)** | Shape `validation/specs/opc-10000-3-address-space/shapes/AS-031-hascomponent-nodeclasses.shacl.ttl`, with 2 passing and 2 failing nodesets under `validation/specs/opc-10000-3-address-space/testcases/AS-031/`. Re-implemented against the emitted vocabulary: the existing `validation/ontology/hasComponent.shacl.ttl` shapes (`HasComponentTargetConstraint`, `HasComponentTargetSourceConstraint`) are written against `base:hasComponent`, which `nodeset2owl.py` does not emit -- it emits `opcua:HasComponent` -- and target `opcua:BaseNodeClass`, which needs RDFS entailment over `base.ttl` to match at all. |
...
| AS-035 | **7.13** | `HasTypeDefinition`: SourceNode must be Object or Variable; TargetNode must be ObjectType (if source is Object) or VariableType (if source is Variable). | **Enforced (suite)** | Shape `validation/specs/opc-10000-3-address-space/shapes/AS-035-type-definition-nodeclass.shacl.ttl`, with 2 passing and 2 failing nodesets under `validation/specs/opc-10000-3-address-space/testcases/AS-035/`. The NodeClass of a type definition is read by hopping from the `owl:Class` back to its defining node via `base:definesType`. *Pass 3: verified* against 7.13 primary text directly — "If the SourceNode is an Object, then the TargetNode shall be an ObjectType; if the SourceNode is a Variable, then the TargetNode shall be a VariableType." Type-consistency half of AS-008's rule (cardinality half). |
