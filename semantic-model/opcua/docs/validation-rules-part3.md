...
described here: `validation/specs/core-part3/` holds a SHACL shape per rule plus
...
| **Enforced (suite)** | A SHACL shape in `validation/specs/core-part3/shapes/` checks this, backed by two passing and two failing NodeSet2 fixtures under `validation/specs/core-part3/testcases/<ID>/`. Written against what `nodeset2owl.py` actually emits and run by `validation/run_suite.py` (also part of `make test`). See `validation/README.md`. |
...
| AS-039 | 6.2.1 | An **instance's** `HasTypeDefinition` target must never be abstract (`IsAbstract=true`) — even though the InstanceDeclaration it derives from may legitimately point to an abstract type. | **Enforced (suite)** | Shape `validation/specs/core-part3/shapes/AS-039-instance-type-not-abstract.shacl.ttl`, with 2 passing and 2 failing nodesets under `validation/specs/core-part3/testcases/AS-039/`. The InstanceDeclaration exemption is carried by the shape: nodes carrying a `HasModellingRule` are exempt. *Pass 2 finding.* "The type of an InstanceDeclaration may be abstract, however the instance must be of a concrete type" (6.2.1). Connects AS-008/AS-010's HasTypeDefinition rules to the `IsAbstract` Attribute from AS-015/AS-017/AS-018/AS-021. |
